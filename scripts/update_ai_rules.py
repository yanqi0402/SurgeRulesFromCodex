...
class RuleList:
    ...
    include_always_real_ip_comment: bool = False
...
def render_always_real_ip_comment(rules: Iterable[RuleEntry]) -> list[str]:
    entries: list[str] = []
    seen: set[str] = set()
    ...
    for rule in rules:
        ...
        if rule.rule_type == "DOMAIN-SUFFIX":
            for entry in (value, f"*.{value}"):
                if entry not in seen:
                    entries.append(entry)
                    seen.add(entry)
        elif rule.rule_type == "DOMAIN" and value not in seen:
            entries.append(value)
            seen.add(value)

    return [
        "# Suggested Fake-IP bypass in [General]:",
        "# `always-real-ip` cannot consume a RULE-SET directly, so keep this line in your main",
        "# Surge profile if you want these domains to resolve to real IPs instead of Fake-IP.",
        f"# always-real-ip = {', '.join(entries)}",
    ]
...
def render_rules(rule_list: RuleList) -> str:
    ...
    provider_results: list[tuple[Provider, ProviderResult]] = []

    for provider in rule_list.providers:
        provider_results.append((provider, resolve_provider_rules(provider)))

    if rule_list.include_always_real_ip_comment:
        combined_rules: list[RuleEntry] = []
        for _, result in provider_results:
            combined_rules.extend(result.rules)
        lines.extend(render_always_real_ip_comment(sort_rules(combined_rules)))
        lines.append("")
